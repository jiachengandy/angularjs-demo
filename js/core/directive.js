define(['core_service'], function()
{
	 ...
	 */
	angular.module("monitor").directive("html", function()
	{
		return {
			restrict : "E",
			controller : function($rootScope, $scope,$location, monitorGet, monitorPost, popup, messageBox,$window,monitorI18n, cfg)
			{
//				// Init Client context. Webapp is "/monitor" for example.
...
//				}
				...
				$rootScope.setCookie = function(key, value)
				{
					...
					document.cookie = key + "="+ escape (value) + ";expires=" + exp.toGMTString()+ ";path=" + monitorCfg.contextPath;
				};
				
				$rootScope.$window = $window;
				$rootScope.$webapp = webapp;
				$rootScope.$get = monitorGet;
				$rootScope.$post = monitorPost;
				$rootScope.$popup = popup;
				$rootScope.$messageBox = messageBox;
				$rootScope.$location = $location;
				$rootScope.$i18n = monitorI18n;
	...
	angular.module("monitor").directive("monitorHref", function()
	{
		return {
			priority : "1",
			restrict : "A",
			compile : function(telement, attr, linker)
			{
				...
				{
					attrs.$observe("monitorHref", function(value)
					{
